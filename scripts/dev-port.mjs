...
if (!existsSync(localDb)) {
  ...
  const mainRoot = wts
    ...
    .trim()
  const srcDb = mainRoot && mainRoot !== root ? join(mainRoot, 'data', 'banking.db') : null
...
// Main checkout always gets 3000. Worktrees hash to 3001..3099 — stable across
// runs (same worktree name → same port) so browser tabs/bookmarks survive.
function portFor(n) {
  if (n === 'banking') return 3000
  const h = createHash('sha1').update(n).digest()
  return 3001 + (h.readUInt16BE(0) % 99)
}
